...
from ai.blog_post_visualize_performance.experiment.text_triplet_lstm_embedding import TextSentimentLSTMEmbeddingExperiment
...
from ai.common.data_provider.text_triplet import TextWordLevelTripletDataProvider
from ai.common.util.embedding import get_embedding_matrix, get_embedding_index
from ai.common.util.io import load_jsonl
from ai.text.tokenize import TwitterGloveTokenizer
from ai.text.transformer.triplet import Vocabulary, \
    TwitterHierarchicalTripletWordLevelTransformer
...
def run_experiment(experiment_directory, embedding_file_path, training_data_file_path, vocabulary_file_path=None, max_document_length=None, max_token_length=None):
    tokenizer = TwitterGloveTokenizer()
    ...
    if vocabulary_file_path:
        ...
        all_tokens = flatten(tokenized_samples)
        max_document_length, max_token_length = get_max_length(tokenized_samples)
        vocabulary.fit(all_tokens)

    vocab_size = len(vocabulary)

    embedding_index = get_embedding_index(embedding_file_path)
    embedding_dimensions = len(list(embedding_index.values())[0])

    embedding_matrix = get_embedding_matrix(
        embedding_dimensions=embedding_dimensions,
        embedding_index=embedding_index,
        token_index_mapping=vocabulary.dictionary
    )

    experiment = TextSentimentLSTMEmbeddingExperiment(experiment_directory=experiment_directory,
    ...
    transformer = TwitterHierarchicalTripletWordLevelTransformer(vocabulary=vocabulary, tokenizer=tokenizer)

    log_dir = os.path.join(experiment_directory, experiment_name)
    encoder, triplet = experiment.get_model()

    training_data_provider = TextWordLevelTripletDataProvider(
    ...
    embedding_data_provider.store_embedding_data(training_data_provider.testing_samples[0:1000])
    joblib.dump(experiment_directory, "vocab.pkl")


def main(experiment_directory, embedding_file_path, training_data_file_path, vocabulary_file_path):
    start = time.time()
    run_experiment(experiment_directory, embedding_file_path, training_data_file_path, vocabulary_file_path)
    end = time.time()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--embedding_file_path", required=True)
    parser.add_argument("--training_data_file_path", required=True)
    parser.add_argument("--vocabulary_file_path", required=False)

    args = parser.parse_args()
    main(args.experiment_directory, args.embedding_file_path, args.training_data_file_path, args.vocabulary_file_path)
